...
fig_path_data = os.path.join(fig_path, 'Nuclear_Pore_complex2',\
...
ker_FP, ker_BP = [], []
ker_init = imread(fig_path_data, 'kernel_init.tif')
ker_true = imread(fig_path_data, 'kernel_true.tif')
ker_FP.append(imread(fig_path_data, 'kernel_fp.tif'))
ker_BP.append(imread(fig_path_data, 'kernel_bp.tif'))

Sx_t, Sy_t, Sz_t = ker_true.shape
Sx_f, Sy_f, Sz_f = ker_FP[0].shape
Sx_b, Sy_b, Sz_b = ker_BP[0].shape

vmax_psf, color_map_psf = 0.01, 'hot'

x_mt  = imread(os.path.join(fig_path_data, f'sample_{id_data}', 'kernelnet'), 'x.tif')
y_mt  = imread(os.path.join(fig_path_data, f'sample_{id_data}', 'kernelnet'), 'y.tif')
...
# axes[0].plot(ker_true[Sx_t//2, :, Sz_t//2], color='black', label='True')
...
# axes[1].plot(ker_true[:, Sy_t//2, Sz_t//2], color='black', label='True')
